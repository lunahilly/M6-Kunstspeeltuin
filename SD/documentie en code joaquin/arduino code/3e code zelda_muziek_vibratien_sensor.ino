...
int vibrationSensor = 2; // Change this to the pin where you have connected your vibration sensor

// notes of the melody followed by the duration.
// a 4 means a quarter note, 8 an eighteenth, 16 sixteenth, and so on
// negative numbers are used to represent dotted notes, so -4 means a dotted quarter note (a quarter plus an eighteenth)
int melody[] = {
...
void loop() {
  ...
  if (digitalRead(vibrationSensor) == LOW) {
    playMelody();
    delay(1000); // Adjust this delay to control how often the melody can be triggered
  }
}
...
void playMelody() {
  ...
  // stop the waveform generation after the melody is complete
  noTone(buzzer);
}
